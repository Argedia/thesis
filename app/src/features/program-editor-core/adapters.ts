...
export const collectVariableDeclarations = (
	...
	const visit = (items: EditorBlock[]) => {
		items.forEach((block) => {
			...
			if (block.kind === "for_each") {
				declarations.push({
					id: block.forEachItemDeclarationId ?? `${block.id}-item`,
					name: block.forEachItemName?.trim() || "item",
					color: block.color,
					bindingKind: "declare"
				});
			}
...
export const migrateLegacyBlocksToProgram = (
	...
	id: programId,
	kind: "program",
	statements: blocks.map(migrateLegacyBlock).map(legacyBlockToStatement)
});

const migrateLegacyBlock = (block: EditorBlock): EditorBlock => {
	if (block.kind === "var_operation") {
		const mode = block.variableOperationMode ?? "value";
		if (mode === "assign") {
			return {
